...
private const val TAG: String = "TopicModel"

object TopicModel : Observable() {
    private var mValueDataListener: ValueEventListener? = null
    private var mTopicList: ArrayList<Topic>? = ArrayList()

    private fun getDatabaseRef() : DatabaseReference? {
        return FirebaseDatabase.getInstance().reference.child("Topic")
    }
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                try {
                    ...
                    if (dataSnapshot != null) {
                        ...
                        mTopicList = data
                        Log.i(TAG, "Data updated. There are " + mTopicList!!.size + " entrees in the cache.")
                        setChanged()
                        notifyObservers()
                    }
                } catch (e: Exception) {
